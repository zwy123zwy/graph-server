...
async function mockLlm(state: typeof MessagesAnnotation.State) {
  ...
  if (role === "human" && /我叫什么|我的名字|what'?s my name/i.test(content)) {
    ...
    for (let i = 0; i < messages.length; i++) {
      const m = messages[i];
      const c = m && typeof m === "object" && "content" in m ? String((m as { content: unknown }).content) : "";
      const r = m && typeof m === "object" && "_getType" in m ? (m as { _getType: () => string })._getType() : "";
...
async function main() {
  ...
  const r1 = await graph.invoke(
    { messages: [new HumanMessage("我叫 Bob")] },
    config
  );
  const out1 = r1.messages[r1.messages.length - 1];
  console.log("AI:", out1 && typeof out1 === "object" && "content" in out1 ? (out1 as { content: unknown }).content : out1);
  ...
  console.log("AI:", out2 && typeof out2 === "object" && "content" in out2 ? (out2 as { content: unknown }).content : out2);
