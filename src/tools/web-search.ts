...
async function searchMoark(query: string): Promise<Array<{ name: string; url: string; snippet?: string }> | null> {
...
async function searchKuaisou(query: string): Promise<Array<{ name: string; url: string; snippet?: string }> | null> {
...
async function searchDuckDuckGo(query: string): Promise<{ abstract?: string; abstractUrl?: string; related: Array<{ text: string; url?: string }> }> {
  ...
  if (Array.isArray(data.RelatedTopics)) {
    for (const t of data.RelatedTopics.slice(0, 8)) {
      if (typeof t === "object" && t?.Text) related.push({ text: t.Text, url: t.FirstURL });
    }
  }
  return {
...
async function searchWikipedia(query: string): Promise<Array<{ title: string; desc: string; url: string }>> {
  try {
...
async function runWebSearch(q: string): Promise<string> {
  const parts: string[] = [];
  ...
  if (ddgResult.abstract) {
    ...
    if (ddgResult.abstractUrl) parts.push(`链接: ${ddgResult.abstractUrl}`);
  }
...
export const webSearchTool = tool(
  ...
  },
  {
    name: "web_search",
    description:
      "在互联网上搜索最新信息。当用户询问新闻、实时数据、当前事件、近期信息或你不确定的内容时，应优先调用此工具获取最新结果后再回答。",
    schema: z.object({
