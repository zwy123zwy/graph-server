...
async function extractPdfText(source: { url?: string; path?: string }): Promise<string> {
  ...
  try {
    ...
    if (!text) return "该 PDF 未能提取到文本（可能为扫描件或空白页）。";
...
export const readPdfTool = tool(
  async (input: unknown) => {
    const { url, path } = input as { url?: string; path?: string };
    if (!url?.trim() && !path?.trim()) return "请提供 url 或 path 之一。";
    return extractPdfText({ url, path });
  },
  {
    name: "read_pdf",
    description:
      "根据 URL 或本地文件路径读取 PDF 并提取正文文本。当用户要求总结、解读或回答与某份 PDF 相关的问题时，先调用此工具获取内容再回答。",
    schema,
  }
);
