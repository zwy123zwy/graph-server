 ...
 */
export async function pruneUploadsLru(
  dir: string,
  maxCount: number = DEFAULT_MAX_FILES
): Promise<void> {
  ...
  const filesWithMtime: { file: string; mtimeMs: number }[] = [];
  for (const name of entries) {
    const fullPath = path.join(resolvedDir, name);
    try {
      const s = await stat(fullPath);
      if (s.isFile()) filesWithMtime.push({ file: fullPath, mtimeMs: s.mtimeMs });
    } catch {
      // 忽略无法 stat 的项
    }
  }

  if (filesWithMtime.length <= maxCount) return;
