...
async function nodeA(state) {
    return { messages: [new AIMessage(`Echo: ${state.messages[0]?.content ?? ""}`)] };
}
async function nodeB(state) {
    const last = state.messages[state.messages.length - 1];
    const text = last && typeof last === "object" && "content" in last ? String(last.content) : "";
...
async function main() {
    ...
    for await (const chunk of await graph.stream(input, { streamMode: "values" })) {
        ...
        const content = msg && typeof msg === "object" && "content" in msg ? msg.content : "";
