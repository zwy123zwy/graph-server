 ...
 */
export function scanContentForAgents(content, filePath = "test.ts") {
    const agents = [];
 ...
 */
async function scanFileForAgents(filePath) {
    try {
 ...
 */
async function findTsJsFiles(dir, files = []) {
    const entries = await fs.readdir(dir, { withFileTypes: true });
    for (const entry of entries) {
        const fullPath = path.join(dir, entry.name);
        // Skip node_modules, dist, .git, etc.
        if (entry.isDirectory() &&
            !["node_modules", "dist", ".git", ".turbo", "build", "coverage"].includes(entry.name)) {
 ...
 */
export async function generateConfig(targetPath) {
    ...
    if (files.length === 0) {
        ...
        outro(color.yellow("No TypeScript or JavaScript files found in the current directory."));
        return;
    }
    ...
    if (unexportedAgents.length > 0) {
        console.log();
        console.log(color.yellow(`⚠️  Found ${unexportedAgents.length} agent(s) that are not exported:`));
        for (const agent of unexportedAgents) {
            const relativePath = path.relative(targetRootPath, agent.filePath);
            console.log(color.dim(`   • ${color.white(agent.name)} at ${relativePath}:${agent.lineNumber}`));
        }
        console.log(color.dim(`   Add ${color.cyan("export")} keyword to include them in the configuration.`));
        console.log();
    }
