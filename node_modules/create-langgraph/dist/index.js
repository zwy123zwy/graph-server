...
import { intro, outro, select, text, isCancel, cancel, confirm, } from "@clack/prompts";
...
const getPmSpecificCommands = () => {
    const npmExecPath = process.env.npm_execpath;
    // default to yarn, as most examples include yarn.lock
    if (!npmExecPath)
        return { install: "yarn install", exec: "npx" };
    const npmBin = path
    ...
    if (npmBin === "yarn")
        return { install: "yarn install", exec: "npx" };
    if (npmBin === "pnpm")
        return { install: "pnpm install", exec: "pnpm dlx" };
    if (npmBin === "bun")
        return { install: "bun install", exec: "bunx" };
    return { install: "npm install", exec: "npx" };
};
// Generate template IDs programmatically
const TEMPLATE_ID_TO_CONFIG = Object.entries(TEMPLATES).reduce((acc, [name, versions]) => {
...
async function downloadAndExtract(url, targetPath) {
    try {
        ...
        const extractedDir = (await fs.readdir(targetPath)).find((f) => f.endsWith("-main"));
        if (extractedDir) {
            const fullExtractedPath = path.join(targetPath, extractedDir);
            const files = await fs.readdir(fullExtractedPath);
            await Promise.all(files.map((file) => fs.rename(path.join(fullExtractedPath, file), path.join(targetPath, file))));
...
export async function createNew(projectPath, templateId) {
    if (templateId) {
    ...
    const guide = language === "js"
        ? color.cyan(dedent `
          ...
          `)
        : null;
    outro([`Project created successfully at ${color.green(absolutePath)}`, guide]
        .filter(Boolean)
        .join("\n\n"));
}
